...
namespace SpeedTest
{
    public class STApi
    {
        ...
        /// <returns></returns>
        public static Int64 Generate(STServer server, int download, int upload, int ping)
        {
            ...
            http.Referer = "http://c.speedtest.net/flash/speedtest.swf?v=307120";
            ...
            var sreceived = (download * 1024).ToString(CultureInfo.InvariantCulture);
            var ssent = (upload * 1024).ToString(CultureInfo.InvariantCulture);
            ...
            using (var resp = (HttpWebResponse)http.GetResponse())
            {
                using (var sr = new StreamReader(resp.GetResponseStream()))
                {
                    ...
                    return Convert.ToInt64(parms["resultid"]);
                }
            }
        }
